...
def callback(data):
    ...
    for i in range(int(num_detections)):
        if Myclasses.has_key(str(int(classes[i]))):
        # scale box to image coordinates
            if classes[i] == 1:
                personNuber = personNuber + 1
            box = boxes[i] * np.array([image.shape[0], image.shape[1], image.shape[0], image.shape[1]])
            left,top,right,bottom = box[1],box[0],box[3],box[2]
            sort_list.append([left,top,right,bottom,scores[i]])
            print(sort_list)
    track_bbs_ids = mot_tracker.update(np.array(sort_list))
    for track in range(len(track_bbs_ids)):
        DeArray.append(Detection2D(classes[track],
                                   float(track_bbs_ids[track][4]),
                                   BoundingBox2D(track_bbs_ids[track][0],
                                                 track_bbs_ids[track][1],
                                                 track_bbs_ids[track][2],
                                                 track_bbs_ids[track][3])))
